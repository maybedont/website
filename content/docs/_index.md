...
```bash
# Using Docker
docker pull ghcr.io/maybedont/maybe-dont:v0.5.8

# Using Podman
podman pull ghcr.io/maybedont/maybe-dont:v0.5.8
```
...
The latest container image is available at `ghcr.io/maybedont/maybe-dont:v0.5.8`.
...
docker run \
  ...
  -v $(pwd)/config:/config \
  -p 8080:8080 \
  ghcr.io/maybedont/maybe-dont:v0.5.8 start --config-path /config
